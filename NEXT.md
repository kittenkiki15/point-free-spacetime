...
1. **予想 C-0001 の主張 4（ローレンツ共変性と局所性）の、点の場合の検証**。
   - 主張：ミンコフスキー時空上の関係 $`T`$（「区別できないほど近い」）が、ポアンカレ変換と可換で（共変）、各点の $`T`$ 近傍 $`T[x]`$ が有界（局所的）なら、$`T`$ は恒等関係に含まれる。したがって、$`T`$ から作る膨張は余白付きの包含の補間性を破れず、最小の尺度を入れられない。
   ...
   - 注意：点なし版（開集合フレーム上の膨張）への持ち上げと、「局所的」の点なしの定義は、その次の課題にする。
