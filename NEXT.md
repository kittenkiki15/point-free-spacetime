...
1. **予想 C-0001 の見直し**：第 06 回の調査メモをもとに、C-0001（特に主張 4 と、「区別」の定義）をユーザーと見直し、予想ファイルと Issue #8 を改訂する。主張 4 の点の場合の証明と Lean での形式化は、見直しの結果に応じて行う。
   ...
   - ユーザーの指摘「『宇宙』の意味を問題とする物理現象に限れば、制限はさらに強まる」は、「現象ごとに使える資源（領域の大きさ、時間、エネルギー）が決まる」という意味（ユーザーに確認済み。6.7 節の注意 3）。資源の上限は観測者側のデータ $`O`$ に入れ、宇宙の大きさ（と年齢）は、共通の時刻の範囲や資源の上限を課した、許される観測者の集合にわたる上限とする。$`R_O`$ の定量的な議論は、当面ミンコフスキー時空に限る。
