...
1. **先行研究の調査：最小の長さとローレンツ共変性、「区別できる」ことの操作的な意味**。予想 C-0001 を見直す前に行う（第 05 回の後のユーザーの判断）。
   ...
   - Claude の見立て（**未確認の知識に基づくので、調査で確かめる**）：
     - 主張 4 の証明の方針が使うのは、**原点以外の各ベクトルのローレンツ軌道が有界でないこと**（ローレンツ群が非コンパクトで、いくらでも大きなブーストがあること）と、その軌道全体を含むミンコフスキー時空を前提にすることである。共変性そのものは数学的な対称性の要請であり、任意の大きさのブーストを物理的に実現できることは要しない。
     - 一方で、物理的に実現できる観測者（有限のエネルギーで到達できるブースト）に対称性を制限すべきかどうかは、別の問いとして調べる。これは最小の長さとローレンツ不変性の緊張関係の一つの形と考えられる（`hossenfelder2013` の 4.1 節は、二重特殊相対論、固有距離としての解釈、エネルギーの尺度としての解釈を挙げている。この節は原典で確認済み）。
