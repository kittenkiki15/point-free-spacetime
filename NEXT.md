# 次のセッションでやること

最終更新: 2026-09-25（第 03 回セッションの終わり）

## 状況

- 第 01 回：リポジトリの運用ルールを決め、自動検証（Lean・Python・クロスモデルレビュー）を整えた。
- 第 02 回：点なし位相と論理・様相論理・因果・量子論のつながりを整理し、調査メモ・用語一覧・記号一覧・参考文献一覧を作った（[まとめ](summaries/2026-09-25_02_pointfree-logic.md)）。
- 第 03 回：Issue #4（PR #3 のレビューの残り）に対応した。Heunen–van der Schaaf の 2 本の論文メモを作り、「完備ブール代数の点はアトムと対応する」を Lean で形式化した（[まとめ](summaries/2026-09-25_03_ordered-locales.md)）。
  - 論文メモ：[heunen2024](surveys/2026-09-25_03_heunen2024.md)、[heunen2026](surveys/2026-09-25_03_heunen2026.md)。
  - Lean：[`lean/PointFreeSpacetime/BooleanPoints.lean`](lean/PointFreeSpacetime/BooleanPoints.lean)。
  - この回はユーザーとの対話による評価や決定をしていないので、まとめの「新しい予想」の見立てはまだユーザーと確認していない。

## 次のタスク（この順で、1 セッションに 1 件ずつ進める）

第 03 回の終わりに、**1 セッション 1 タスク**の運用に改めた（[`CLAUDE.md`](CLAUDE.md) の「セッションの進め方」）。次のセッションでは、下のタスク 1 だけを扱う。

1. **第 03 回の結果をユーザーと確認する**。
   - 論文メモの「調査メモ 7 節の予想の候補は、既に扱われているか」（[heunen2026 のメモ](surveys/2026-09-25_03_heunen2026.md)）と、まとめの「新しい予想」の見立て。
2. **最初の予想を立てる**。候補は次の 3 つ（詳細は heunen2026 のメモ）。確度・重要度・検証費用は Claude が案を出し、優先度はユーザーと相談して決める。
   - 候補 1（点なしの時制論理）：(c-∨)（局所的な錐が任意の結びを保つ）と平行性を満たす順序付きロケールに方向性の公理を加えると、その時制論理が S4.2（に相当するもの）になるか。ただし比べる前に、**命題をどこで評価するか**（時空の点の任意の集合か、開集合フレームか）、**扱う様相と言語**（Goldblatt と同じく未来の □ だけの単一様相か、過去・未来の両方か）、**妥当性の定義**を決める必要がある。開集合フレームで評価すると排中律が一般には成り立たないので、比べる相手は古典的な S4.2 そのものではなく、その直観主義版や、古典論理で表せる断片になる可能性がある。あわせて、$`↟`$・$`↡`$ の共役性と Jónsson–Tarski の共役の関係を、フレーム（ハイティング代数）の場合に確かめる。
   - 候補 2（点がないと表せない因果構造）：点を持たない順序付きロケールの因果的な性質が、どの点のある時空とも違うか。
   - 候補 3（量子と時空の点のなさの統一）：今回読んだ Heunen–van der Schaaf の 2 本では扱われていない。他の先行研究は未調査。
   - 登録するときは `conjectures/C-NNNN.md` と一覧表を作り、Issue を立てる。
3. **追加の調査**（予想の検討に必要になったら）：van der Schaaf の博士論文（`vanderschaaf2024`、arXiv: 2406.15406）、Jónsson–Tarski（`jonsson1951`）、Arntzenius（`arntzenius2003`）。

## ユーザーにお願いしていること

- [ ] （任意）追加の調査候補の PDF の入手：`vanderschaaf2024`（arXiv で公開）、`jonsson1951`、`arntzenius2003`。heunen2026 の出版社版（手元は arXiv 版 v1）。
- [ ] （任意）`main` ブランチの保護ルールで、PR と CI の成功を必須にする。

## 未解決の論点

- 基礎の調査メモの未確認事項（正則開集合の例の典拠、全射の例の候補の正確な定式化と典拠など）。
- heunen2024 の例 6.6（一点を除いたミンコフスキー時空が $`T_0`$ 順序でない）の具体的な理由（論文メモに Claude の理解を記録した）。
