...
1. **先行研究の調査：最小の長さとローレンツ共変性、「区別できる」ことの操作的な意味**。予想 C-0001 を見直す前に行う（第 05 回の後のユーザーの判断）。
   ...
   - Claude の見立て（**未確認の知識に基づくので、調査で確かめる**）：
     ...
     - 「区別」は、特定の観測者が、試験粒子（プローブ）を当てて、有限の時間をかけて行うと考えられる。観測者は 4 元速度 $`u`$ だけでは指定できず、世界線、測定にかける時間、プローブの状態と測定の手順も要る。そうであれば、単一の不変な $`T`$ ではなく、観測者やプローブのエネルギーを添字にした関係の族 $`T_{u, E}`$ を、族として共変に扱うべきかもしれない。また、区別の対象は点ではなく、状態や領域である。
   - 調べること（候補）：
