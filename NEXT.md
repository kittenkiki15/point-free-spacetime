...
1. **予想 C-0001 の見直し**：第 06 回の調査メモをもとに、C-0001（特に主張 4 と、「区別」の定義）をユーザーと見直し、予想ファイルと Issue #8 を改訂する。主張 4 の点の場合の証明と Lean での形式化は、見直しの結果に応じて行う。
   ...
   - 検討する主張の形（6.7 節。ユーザーが賛同）：観測者ごとに、紫外の下限 $`ℓ`$（実現できる領域の族に課す、候補となる許容条件の尺度。最小の実効的な分解能とみなすには、結合の実効的な支持や誘導される観測量への追加の条件が要る）と赤外の上限（到達できる範囲 $`R_O`$。観測者の有限な世界線の区間の両端の事象を頂点とする因果ダイヤモンドの、両端を結ぶ時間的なベクトルの静止系での空間の半径。両端の間の固有時間を $`τ_O`$ として $`R_O = τ_O/2`$）を持つ、実現できる領域の族を考える。族全体は共変だが、各観測者の族から作る点なしの構造（局在の関係、余白付きの包含）には、$`ℓ`$ と $`R_O`$ から決まる実効的な尺度が現れる。主張 4 との関連は $`R_O → ∞`$ の極限で調べる（極限でも観測者の添字は残るので、主張 4 に接続するには、観測者への依存が消えるなどの追加の条件が要る。PR #11 のレビューの指摘）。
