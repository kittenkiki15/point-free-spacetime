...
| singular state | 特異状態 | 連続な観測量の値の範囲を表す射影の代数（測度 0 の違いで同一視した領域の代数）に、確率 0 か 1 だけを割り当てる状態。点の値に対応するが、可算加法性を破る（Halvorson、Arntzenius の紹介による）。代数の言葉では、ロケールの点でない超フィルターにあたる。 | [heunen2026 のメモ](surveys/2026-09-25_03_heunen2026.md) |
