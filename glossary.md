...
| impossible measurement | 不可能な測定 | 因果律と矛盾する（光より速い信号を送れてしまう）測定。Sorkin（1993）が提起した。Bostelmann–Fewster–Ruep（2021）は、同論文の仮定（コンパクトな結合領域、因果的な順序付け、因果的な因子分解など）を満たす局所的な結合では、検討した Sorkin 型の信号は生じないことを示し、「不可能な測定には、不可能な（非局所的な）装置が要る」とまとめた。 | [第 06 回の調査メモ](surveys/2026-09-26_06_minimal-length-covariance.md) |
