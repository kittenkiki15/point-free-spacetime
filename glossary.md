...
| de Vries algebra | de Vries 代数 | 完備ブール代数に、補間性を満たす近接関係を加えたもの。コンパクト・ハウスドルフ空間と双対になる（de Vries の双対性）。 | [第 05 回の調査メモ](surveys/2026-09-26_05_subordination.md) |
...
| interpolation (of a relation) | 補間性 | 関係 ◁ について、a ◁ b ならば a ◁ c ◁ b となる c があること。余白付きの包含では「余白を二つに分けられる」ことにあたる。長さ空間の ℓ 近傍による膨張では、ℓ > 0 だと破れる（一般には、補間性の破れと最小の尺度は一致しない）。 | [第 05 回の調査メモ](surveys/2026-09-26_05_subordination.md) |
