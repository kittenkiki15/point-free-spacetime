...
| induced observable | 誘導される観測量 | Fewster–Verch の測定の枠組みで、プローブの観測量の測定を、系の観測量の測定とみなしたもの。プローブの初期状態と結合に依存し、プローブの測度が射影値（鋭い）でも、誘導される系の測度は一般に射影値にならない（原典の表現では、誘導される観測量は一般にプローブの観測量より鋭くない）。 | [第 06 回の調査メモ](surveys/2026-09-26_06_minimal-length-covariance.md) |
