...
| induced observable | 誘導される観測量 | Fewster–Verch の測定の枠組みで、プローブの測定の結果の確率（期待値）を、系の上で表す観測量。測定後の状態の更新は、これとは別に（測定器具として）指定する必要がある。プローブの初期状態と結合に依存し、プローブの測度が射影値（鋭い）でも、誘導される系の測度は一般に射影値にならない（原典の表現では、誘導される観測量は一般にプローブの観測量より鋭くない）。 | [第 06 回の調査メモ](surveys/2026-09-26_06_minimal-length-covariance.md) |
...
| observer independence | 観測者非依存性 | 最小の長さなどの主張が、観測者（慣性系）の選び方に依らないこと。Hossenfelder（2006）は、合意した初期条件で同じ実験をしたときに、どの静止系でも結果が同じになることとして定める。 | [第 06 回の調査メモ](surveys/2026-09-26_06_minimal-length-covariance.md) |
