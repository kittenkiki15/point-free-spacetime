...
| singular state | 特異状態 | ブール代数（σ-完備）上の状態のうち、可算加法的（正規）でないもの（Halvorson）。2 値とは限らない。ここで主に扱う**特異な純粋状態**は、連続な観測量の値の範囲を表す射影の代数（測度 0 の違いで同一視した領域の代数）に確率 0 か 1 だけを割り当てる状態で、点の値に対応するが可算加法性を破る。特異な純粋状態は、ロケールの点でない超フィルターにあたる。 | [heunen2026 のメモ](surveys/2026-09-25_03_heunen2026.md) |
