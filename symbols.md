...
| $`ℓ`$、$`R_O`$ | 紫外の下限、赤外の上限 | 実現できる領域の最小の大きさと、観測者 $`O`$ が使える時空領域（有限な世界線の区間の両端の事象を頂点とする因果ダイヤモンド）の、両端を結ぶ時間的なベクトルの静止系での大きさ | | [調査 06](surveys/2026-09-26_06_minimal-length-covariance.md) |
